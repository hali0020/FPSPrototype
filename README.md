...
- 移动、空中和持续连射会分别增加弹道扩散，准星会按当前扩散动态张合
...
- 项目内确定性生成的步枪开火音效，并使用 8 声部音频池保留连射尾音
...
该枪声只使用固定随机种子和数学合成生成，不包含下载、录制或第三方音频采样。枪上的全息瞄具外框同样由项目程序化生成；武器主体仍是 Epic/UE 模板步枪，并不是尚未审计的 Deadghost 武器包。
