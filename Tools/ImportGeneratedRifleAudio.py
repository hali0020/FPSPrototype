...
source_wav = os.path.join(
    project_dir, "SourceAssets", "Audio", "SFX_Rifle_Shot_01.wav"
)
destination_path = "/Game/Weapons/Rifle/Audio/Generated"
expected_object_path = (
    destination_path + "/SFX_Rifle_Shot_01.SFX_Rifle_Shot_01"
)

if not os.path.isfile(source_wav):
    raise RuntimeError(f"Generated rifle WAV was not found: {source_wav}")

task = unreal.AssetImportTask()
task.filename = source_wav
task.destination_path = destination_path
task.destination_name = "SFX_Rifle_Shot_01"
task.automated = True
...
unreal.log(
    "Imported generated rifle report: "
    + source_wav
    + " -> "
    + expected_object_path
)
